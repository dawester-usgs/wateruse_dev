	...
	$.ajax ({
			...
			success:function(data){
				...
				$("#the-anndata").append('<thead>\
														<tr>\
														<td><input type="checkbox" id="the-mpid-selectall" disabled="disabled"></td>\
														<td>MPID</td>\
														<td>Name</td>\
														<td>Use Type</td>\
														<td>Paid</td>\
...
$(document).ready(function(){
	...
	$("#filterbycounty").click(function(){
			...
			if (check){
			$("#x").val('sc');
				$(".tt-menu").text(decodeURI(" Please hit Enter or Return key"));
			}else{
				$("#x").val('');
				//$('.typeahead').typeahead('val', '');
				$(".tt-menu").text(decodeURI(" Please hit Enter or Return key"));
	...
	$("#add-new-mpid").click(function(){
		...
		if (clickedmeyear == false || clickedmemethod == false){
			if (!confirm('Warning: You must select a year, method, and un/check the unpaid to continue otherwise the default values for Water Year is the current year, Method is Estimated and Unpaid. Would You like to continue?')){
				return false;
			}else{
					...
					<div class="input-group-btn" >
						...
						<div class="btn-group"> 
									<label class="btn btn-primary">
										<input type="checkbox" id="filterbycounty"> Filter by my County 
									</label>
						</div>
						
					</div> 
				<div id="scrollable-dropdown-menu">					
					<input type="text" class="form-control typeahead tt-query" id="search-box" placeholder="ID or Name of:"/>
				</div>
...
<!-- table goes here -->
  ...
  <div class="modal-dialog" role="document" style="z-index:1;">
	...
	<!--	<tr><td><input type='checkbox'  name='county_cd' id='the-county'  class="the-selection" > <span id='my-county'> My County </span></td></tr> -->
		...
		<div id="add-data">
			<form method='POST' target='__blank' id='form-add-data' >
