		...
		#	print $sqlQuery1;
			...
			while ($arr = oci_fetch_array($parser,OCI_BOTH)){
				...
				#	print_r ($rows);
					...
					$dom_no = $rows['dom_del_amt'];
					
					$com_del_amt = $rows['com_del_amt']; 
					$com_no = $rows['com_del_amt']; 
...
class update {
		...
		if ($newDiverter && !$newOwner){
			...
			######################################################################################################################
			   ####					####NOTE TO SELF. TRY THIS ONE MORE TIME WITH MORE ENHANCE MEMORY LIMIT ##########################
				...
				$getOwnernm = !is_null($newOwner) ?  str_replace("'", "''",$validate->validate_ID('Owner', $newOwner)): $getOwnernm = "";
				$getDiverternm = !is_null($newDiverter) ?  str_replace("'", "''",$validate->validate_ID('Diverter', $newDiverter)) : $getDiverternm = "";
				...
				for($i = 0; $i<=$arrCtr; $i++){
					...
					$checkRelay  = "SELECT * FROM STATION_RELAY
						...
						$stid = oci_parse($conn, $checkRelay);
						...
						if ($arr['new_owner'] && $arr['new_diverter']){
							
							$movetype = $arr['hasData'.$i] == 'Yes' ? $movetype = "COMPLETE_STATION_ANNDATA" : $movetype="COMPLETE_STATION";
						}else{
							$movetype = $arr['hasData'.$i] == 'Yes' ? $movetype = "STATION_ANNDATA" : $movetype = "STATION_ONLY";
						}
						...
						if ($rows_returned == 0 ){
							$sqlStation = "INSERT INTO
											...
											VALUES(
												...
												'".$arr['owner_id'.$i]."',
												'".$arr['diverter_id'.$i]."', 
												null, /* (new_fac_id) change this to facility later */
												null, /* (new_fac_nm)  change this to facility later */
												null, /* (old_fac_id)change this to facility later */
												'".$arr['year']."',
												'".$movetype."'
						...
						$newDiverterSQLRelay2 = $newDiverter == "" ? $newDiverterSQL ="": " old_diverter_id = '".$arr['diverter_id'.$i]."',";
						$newOwnerSQLRelay2 = $newOwner == ""  ? $newOwnerSQL ="" : " old_owner_id = '".$arr['owner_id'.$i]."',";
						
						
						$newDiverterSQLRelayConds = $newDiverter == "" ? $newDiverterSQL ="": " OLD_DIVERTER_ID = '".$arr['diverter_id'.$i]."' AND ";
						$newOwnerSQLRelayConds = $newOwner == ""  ? $newOwnerSQL ="" : "OLD_OWNER_ID = '".$arr['owner_id'.$i]."' AND";
							...
							$sqlStation = "UPDATE 
										...
										SET 
											 ...
											 MNAME = '".$_SESSION['USER_ID']."',
												...
												WYEAR = '".$arr['year']."'
										WHERE 
											/*".$newDiverterSQLRelayConds."
											 ".$newOwnerSQLRelayConds."			*/					
											MPID = '".$arr['multimpid'.$i]."' 	AND									
											WYEAR = '".$arr['year']."' 								
											 ";
						}
						...
						#	print $sqlStation.'=S=====================';
					if ($errStation){
						## update ann_data to prevent bottleneck ##
						$success = true;
						$proceedToAnnData =true;
					}
				}	
					...
					if ($proceedToAnnData){
						...
						for($i = 0; $i<=$arrCtr; $i++){
							...
							if ($errAnnData){
								## update exdata to prevent bottleneck ##
								$success = true;
								$proceedToExData = true;
							}	
						}
	...
	/*	if (count(preg_grep_key('/^new-crop_cd/', $arr)) === 0) {
			...
			$k = $tae;
					...
					$total_monthly = $convert_this->convert_anndata($units,$arr['total_widthdrawn']);
		...
		#	
				...
				$sqlAnnData ="UPDATE ANN_DATA
									SET	
										...
										ANN_DATA.ENT_ANN_AMT = '".$arr['total_widthdrawn']."',
