...
<script>
	...
	function appendDynamic(){
		...
		$("#the-anndata").append('<tbody>\
													<tr id="row'+i+'">\
														<td>'+i+' </td>\
														<td><select name="crop_cd'+i+'" id="crops'+i+'" class="form-control ann_dataArr crops_cd"></select></td>\
														<td><input type="text" name="acres'+i+'" class="masked form-control ann_dataArr AnnDataNumbers"  id="acres'+i+'"  placeholder="0" onkeypress="return isNumberKey(event,this.value)"  placeholder="0" min="0"/></td>\
														<td><input type="text" name="rate'+i+'"  class="masked form-control  ann_dataArr AnnDataNumbers" id="rate'+i+'"  placeholder="0" onkeypress="return isNumberKey(event,this.value)" placeholder="0"   min="0"/></td>\
														<td><select name="method'+i+'" id="method'+i+'" class="form-control required"></select></td>\
														<td class="crops_meterReading"><input type="text" class="masked ann_dataArrform-control" readonly></select></td>\
														<td><input type="text" name="total'+i+'" class="masked ann_dataArr form-control total"  id="total'+i+'"  placeholder="0" onkeypress="return isNumberKey(event,this.value)"/></td>\
		...
		getCropsandMethod(i);	
			...
			$("#the-form").append('<input type="hidden" name="id" id="id_ctr" class="ann_dataArr" value="1">');
				...
				i+=1;
						...
						$("#the-anndata").append('<tbody>\
													<tr id="row'+i+'">\
														...
														<td><input type="text" name="acres'+i+'" class="masked form-control ann_dataArr AnnDataNumbers"  id="acres'+i+'"  placeholder="0" onkeypress="return isNumberKey(event,this.value)"  placeholder="0" min="0"/></td>\
														<td><input type="text" name="rate'+i+'"  class="masked form-control  ann_dataArr AnnDataNumbers" id="rate'+i+'"  placeholder="0" onkeypress="return isNumberKey(event,this.value)" placeholder="0"   min="0"/></td>\
														<td><select name="method'+i+'" id="method'+i+'" class="form-control ann_dataArr required"></select></td>\
														<td class="meter-reading"><input type="text" class="masked ann_dataArr form-control"></select></td>\
														<td><input type="text" name="total'+i+'" class="masked ann_dataArr form-control total"  id="total'+i+'"  placeholder="0" onkeypress="return isNumberKey(event,this.value)" readonly/></td>\
														<td class="text-warning text-center eh .meter-reading "></td>\
														<td class="text-warning text-center eh meter-reading"></td>\
													</tr>\
													</tbody>');
	...
	$("#crops"+n).on('change', function(){
		 ...
		 $("#rate"+n).on('blur',function(){
			...
			if (rate != ""){
				if (total == 0){
					...
					$("#rate"+n).val(rate.toFixed(2));
				}else if (acres == 0){
					acres = parseFloat(total /rate);
					$("#acres"+n).val(acres.toFixed(2));
				}else{
					total = parseFloat(acres * rate);
					$("#total"+n).val(total.toFixed(2));
				}
		...
		$("#total"+n).on('blur',function(){
			...
			if (total == 0){
				...
				$("#acres"+n).val(tots.toFixed(2))
				
			}else if (acres !="" || acres !=0){
				tots = parseFloat(total/acres);
				$("#rate"+n).val(tots.toFixed(2))
			}
...
$(document).ready(function(){
    ...
    $(this).keypress(function (e) {
		...
		$("#the-form").submit(function(e){
			...
			// var adv_form = $('#the-form').filter(function(index, element) {
									// return $(element).val() != '';
								// });
			...
			if (getNext == "Y"){
				...
				//alert(total_monthly);
						...
						if (proceed == true){
							...
							var data = $("#the-form .ann_dataArr").serialize();
							var basicInfo = $("#the-form .basicInfo").serialize();
							...
							var diditsavecrops = false;
							var diditsaveemall = false;
								$.ajax({
									url:"../controller/wells/savecrops", 
									type:"POST",
									data:data,
									success:function(data){
										if(data == 'success'){
								...
								if (diditsavecrops = true){
									var data = $("#the-form .ann_dataArr2").serialize();
									data = data+"&"+basicInfo;
									$.ajax({
										url:"../controller/wells/saveanndata2", 
										type:"POST",
										data:data,
										success:function(data){
											if(data == 'success'){
...
<body>
 ...
 <?php #print_r ($_GET['thempid']); ?>
	...
	<form class="form-horizontal" id="the-form" method="POST">
		<input type="hidden" class="basicInfo" id="mpid" name="mpid" value = "<?php echo $_GET['thempid']; ?>">
		<input type="hidden" class="basicInfo" name="action_cd" value = "<?php echo $_GET['action_cd']; ?>">
		<input type="hidden" class="basicInfo" name="source_cd" value = "<?php echo $_GET['source_cd']; ?>">
		<input type="hidden" name="id" id="idh" value = "<?php echo $_GET['id']; ?>">
		<input type="hidden" class="basicInfo" name="type" id="type" value = "<?php echo $_GET['type']; ?>">
...
<!--annual data-->
			...
			<label class="control-label col-md-2" for="name-id">ID:</label>
			 <div class="col-sm-10">
				...
				<input type="hidden" name="owner_id" class="basicInfo" id="owner_id-h" />
			</td>
			</tr>
			...
			<label class="control-label col-md-2" for="diverter-id">ID:</label>
			 <div class="col-sm-10">
				...
				<input type="hidden" name="diverter_id" class="basicInfo" id="diverter_id-h" />
			</td>
			</tr>
			...
			<label class="control-label col-md-2" for="year">Year:</label>
			 <div class="col-sm-10">
				...
				<input type="hidden" name="year" id="year-h" class="basicInfo"  value="<?php echo $_GET['year'];?>"/>
			</div>
			</td>
			</tr>
			<tr>
			...
			<tr class="meter-reading">
				<td colspan=2>
					...
					<div class="col-md-6">
						<input type="number" id="alpha-reading" class="form-control meter-reading-date"> 
						<span class="important">*</span>
				...
				<td colspan=2>
					<label class="control-label col-md-4 for="Method"  style="line-height:35px;">Date for Initial Meter Reading: </label>
					<div class="col-md-6">
						<input type="text" id="alpha-reading-date" class="form-control meter-reading-date"> 
						<span class="important">*</span>
					</div>
				</td>
			...
			<tr class="meter-reading">
				<td colspan=2>
					...
					<div class="col-md-6">
						<input type="number" id="omega-reading" class="form-control"> 
						<span class="important">*</span>
					</div>
				</td>
				<td colspan=2>
					...
					<div class="col-md-6">
						<input type="text" id="omega-reading-date" class="form-control meter-reading-date"> 
						<span class="important">*</span>
					</div>
				</td>
			</tr>
			...
			<tr class="measured-reading">
				<td colspan=4>
					...
					<div class="col-md-3">
						<input type="number" id="tot-meas" class="form-control"> 
			...
			<tr class="measured-reading">
				<td colspan=4>
					...
					<div class="col-md-3">
						<select id="meas-units" class="form-control">
							<option value="ACFT">Acre-Feet</option>
							<option value="MG">Million Gallons</option>
	...
	<!--crops-->
		...
		<table class="table table-bordered table-condensed table-hover" id="the-total" style="position:relative; bottom:20px;">
			...
			<div class="col-xs-6">
				<input type="text" class="form-control ann_dataArr2 monthlies" name="jan" placeholder="0" step='1' onkeypress="return isNumberKey(event,this.value)" onBlur = "return computeMonthlies(this.value,$(this).attr('name'));" />
			</div></td>
			
			<td>
			<label class="control-label col-md-2" for="year">Feb</label>
			<div class="col-xs-6">
				<input type="text" class="form-control ann_dataArr2 monthlies"  name="feb" placeholder="0"  onkeypress="return isNumberKey(event,this.value)" onBlur = "return computeMonthlies(this.value,$(this).attr('name'));isNumberKey(event,this.value);"  />
			</div></td>
			
			<td>
			<label class="control-label col-md-2" for="year">Mar</label>
			<div class="col-xs-6">
				<input type="text" class="form-control ann_dataArr2 monthlies"name= "mar" placeholder="0"  onkeypress="return isNumberKey(event,this.value)" onBlur = "return computeMonthlies(this.value,$(this).attr('name'));isNumberKey(event,this.value);" />
			</div></td>
			</tr>
			...
			<div class="col-xs-6">
			<input type="text" class="form-control ann_dataArr2 monthlies" name="apr" placeholder="0"  onkeypress="return isNumberKey(event,this.value)" onBlur = "return computeMonthlies(this.value,$(this).attr('name'));isNumberKey(event,this.value);" />
			</div></td>
			
			<td>
			<label class="control-label col-md-2" for="year">May</label>
			<div class="col-xs-6">
			<input type="text" class="form-control ann_dataArr2 monthlies" name="may" placeholder="0"  onkeypress="return isNumberKey(event,this.value)" onBlur = "return computeMonthlies(this.value,$(this).attr('name'));isNumberKey(event,this.value);"  />
			</div></td>
			
			<td>
			<label class="control-label col-md-2" for="year">Jun</label>
			<div class="col-xs-6">
			<input type="text" class="form-control ann_dataArr2 monthlies" name="jun" placeholder="0" onkeypress="return isNumberKey(event,this.value)" onBlur = "return computeMonthlies(this.value,$(this).attr('name'));isNumberKey(event,this.value);" />
			</div></td>
			</tr>
			...
			<div class="col-xs-6">
			<input type="text" class="form-control ann_dataArr2 monthlies" name="jul" placeholder="0" onkeypress="return isNumberKey(event,this.value)" onBlur = "return computeMonthlies(this.value,$(this).attr('name'));isNumberKey(event,this.value);" />
			</div></td>
			
			<td>
			<label class="control-label col-md-2" for="year">August</label>
			<div class="col-xs-6">
			<input type="text" class="form-control ann_dataArr2 monthlies" name="aug"  placeholder="0" onkeypress="return isNumberKey(event,this.value)" onBlur = "return computeMonthlies(this.value,$(this).attr('name'));isNumberKey(event,this.value);"  />
			</div></td>
			
			<td>
			<label class="control-label col-md-2" for="year">Sept</label>
			<div class="col-xs-6">
			<input type="text" class="form-control ann_dataArr2 monthlies" name="sept"  placeholder="0" onkeypress="return isNumberKey(event,this.value)" onBlur = "return computeMonthlies(this.value,$(this).attr('name'));isNumberKey(event,this.value);" />
			</div></td>
			</tr>
			...
			<div class="col-xs-6">
			<input type="text" class="form-control ann_dataArr2 monthlies" name="oct"  placeholder="0" onkeypress="return isNumberKey(event,this.value)" onBlur = "return computeMonthlies(this.value,$(this).attr('name'));isNumberKey(event,this.value);"   />
			</div></td>
			
			<td>
			<label class="control-label col-md-2" for="year">Nov</label>
			<div class="col-xs-6">
			<input type="text" class="form-control ann_dataArr2 monthlies" name="nov"  placeholder="0" onkeypress="return isNumberKey(event,this.value)" onBlur = "return computeMonthlies(this.value,$(this).attr('name'));isNumberKey(event,this.value);"  />
			</div></td>
			
			<td>
			<label class="control-label col-md-2" for="year">Dec</label>
			<div class="col-xs-6">
			<input type="text" class="form-control ann_dataArr2 monthlies" name="dec"  placeholder="0" onkeypress="return isNumberKey(event,this.value)" onBlur = "return computeMonthlies(this.value,$(this).attr('name'));isNumberKey(event,this.value);" />
			</div></td>
			</tr>
			...
			<table class="table table-bordered table-condensed table-hover">
				...
				<td>
					...
					<label class="radio-inline">
						<input type="radio" name="restrictions" class="required ann_dataArr2" value="N" required checked>No</label>
						<span class="important">*</span>
				</td>
				...
				<label class="radio-inline">
					<input type="radio" name="salinity" value = "N" class = "required ann_dataArr2" required checked>No</label>
					<span class="important">*</span>
				</td>
				...
				<tr>
					<td colspan=1>
						...
						<label class="radio-inline">
							<input type="radio" name="use_cd" class="basicInfo" value= "AG" class = "ann_dataArr2 required" required checked>Agriculture</label>
					...
					<label class="radio-inline">
						<input type="radio" name="who" id= "who-owner" class = "ann_dataArr2 radio-inline required"  value="O" required> Owner
					</label>
					<label class="radio-inline">
						<input type="radio" name="who" id="who-diverter" class = "ann_dataArr2 radio-inline required" value="D" > Diverter
					</label>
						<span class="important">*</span>
					</td>
				</tr>	
				...
				<tr>
					<td colspan=4>
						<div style="margin-left:37%">
							...
							<input type="submit" class="btn" value="Save Annual data" id="saveann" >
