	...
	// conditions(variable,table name.row);
			...
			if ($raw_use_cd != 'all' && $raw_sic_cd == 'all'){
					...
					if ($isIR == true){
							...
							if ($k <= 2){
								$resultIR= $sql->select($maintable,$whereConditionsSQL,$joinSQL2,"default","default",$joinWhereSQL,$flagDebug);
								if (empty($resultIR)){
					...
					if ($isWS == true){
							...
							$joinSQL11 = $joinSQL.' '.$joinWSupply; //.$joinExdataWS;
					...
					*/
					...
					$a = $mergeResult1;
					print_r (file_put_contents($cachePath, json_encode($a));
					
					$prefix = '';
					echo '[';
					foreach ($mergeResult1 as $rows){
						echo $prefix, json_encode($rows);
						$prefix = ",";
					}
					echo "]";
